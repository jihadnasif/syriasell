...
import 'package:syria_sell/data/datasource/remote/jobs/JobOpportunities.dart';
import 'package:syria_sell/data/model/job/ViewJobOpportunitiesModel.dart';
...
class ViewJopOpportunityControllerImp extends GetxController {
  ...
  ViewFavoriteJobOppData viewFavoriteJobOppData =
      ViewFavoriteJobOppData(Get.find());
  List<ViewJobOpportunitiesModel> dataFavoriteJob = [];
  ...
  goToJobOpportunityId(int jobId) async {
    await Get.toNamed(
  ...
  Future<void> fetchData() async {
    await getJobOpportunities(); // ثم جلب كل الوظائف
  }

  Future<void> getJobOpportunities() async {
    listJobOpportunities.clear();
    statusRequest = StatusRequest.loading;
    update();

    try {
      var response = await jobOpportunitiesData.getAllJobOpportunities();
      ...
      if (StatusRequest.success == statusRequest) {
        if (response["status"] == "success") {
          ...
          for (var job in listJobOpportunities) {
            if (job.jobId != null) {
          ...
          print(
              "All jobs IDs: ${listJobOpportunities.map((e) => e.jobId).toList()}");
          print(
  ...
  Future<void> getMyViewFavorite() async {
    try {
      var response = await viewFavoriteJobOppData
      ...
      if (hindlingData(response) == StatusRequest.success &&
        ...
        dataFavoriteJob = responseData.map((e) {
          ...
          var model = ViewJobOpportunitiesModel.fromJson(e);
  ...
  Future<void> addFavorite(int oppid) async {
    try {
      var response = await viewFavoriteJobOppData.postAddFavorite(
          myServices.sharedPreferences.getInt("idUser")!, oppid);
      if (hindlingData(response) == StatusRequest.success &&
          response["status"] == "success") {
  ...
  Future<void> removeFavorite(int oppid) async {
    try {
      var response = await viewFavoriteJobOppData.postRemoveFavorite(
          myServices.sharedPreferences.getInt("idUser")!, oppid);
      if (hindlingData(response) == StatusRequest.success &&
          response["status"] == "success") {
  ...
  void onInit() async {
    ...
    print("Total jobs: ${listJobOpportunities.length}");
  ...
  Future<ViewJobOpportunitiesModel?> getJobOpportunityById(int jobId) async {
    print("-*********-----1----------");
    try {
      var response = await jobOpportunitiesData.getJobOpportunityId(jobId);
      if (response["status"] == "success" && response["data"] != null) {
        return ViewJobOpportunitiesModel.fromJson(response["data"]);
      }
      print("-*********-----2----------");
    } catch (e) {
